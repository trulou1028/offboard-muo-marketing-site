...
| 043 | Minimal launch: cut `offboard.co` over to this repo this week with a launch set of nine routes, the rest live but `noindex` and out of nav | P0 | M | 042 | PLAN WRITTEN 2026-09-07 on `claude/plan-043-minimal-launch`; answers the owner's "is homepage + pricing + employers enough" (no: add /how-it-works, /about, /privacy-security, /resources, /intake, /act, all already built). Steps 2 to 4 are agent work; 5 and 6 are owner-only. |
| 044 | How It Works rebuilt as the long form of the homepage's four steps (plan 040 finding 1, second half); Gmail-as-live ledger breach fixed first | P1 | M | 040, 043 | PLAN WRITTEN 2026-09-07 on `claude/plan-044-how-it-works-messaging`. Four owner decisions gate the rewrite; step 0 (the ledger hotfix) can ship today. |
